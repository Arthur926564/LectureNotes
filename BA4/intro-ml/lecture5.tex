...
\begin{parag}{Mapping to a higher-dimensional space}
	...
	$ $\\
	Note that it is important for the mapping $\phi$ to be \important{nonlinear}. A linear mapping would not result in a nonlinear model in the original space. To show this, let us consider the following linear mapping from a 2D input $x$ to a 3D representation $\phi\left(x\right)$:
	\begin{align*} 
	...
	\begin{subparag}{Intuition}
		Adding dimension which are linearly depedent from other is '\textit{redundant}'. If $\phi$ was a linear function, then the job of this function could already been made by the model. The goal of adding a dimension was to do the job that the linear model couldn't do. Literaly any linear function can be done by the linear model (by definition).
	\end{subparag}
