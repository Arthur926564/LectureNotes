...
\begin{parag}{Dimensionality Reduction}
	...
	$ $\\
	Another example would be by images created by translating and rotating the same digit $3$
	\begin{center}
...
\begin{parag}{Non centred data}
	...
	\begin{itemize}
	    \item Even if the original data is centred, the data after the mapping to feature space might not be
	\end{itemize}
...
\begin{parag}{Example}
	...
	\begin{center}
	\includegraphics[scale=0.2]{screenshots/2026-06-19.png}
	\end{center}
	\begin{center}
...
\begin{parag}{Dimensionality expansion}
    Because the mapping is nonlinear, there is no reason to restrict (again...) the latent representation to be low dimensional. One can then perform dimensionality expansion 
	\begin{center}
...
\begin{parag}{Denoising autoencoders}
    Having a low-dimensional latent representation encourages the autoencoder to learn an '\textit{intelligent}' mapping. With dimensionality expansion, one could simply learn to copy the input. To avoid this, one can add noise to the input and aim to reconstruct a noise-free version of the input.
	\begin{center}
