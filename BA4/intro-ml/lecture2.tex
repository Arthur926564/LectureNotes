...
\begin{parag}{Plane}
	...
	All the thing we said to a one dimension fitting is translated into two dimension: Given $N$ noisy points $\left\{\left(x_i, y_i\right)\right\}$, where $x_i \in \mathbb{R}^2$, find the plane that best fit these observations. Of course as you could have guess, this can be generalized to higher dimensions.
	\begin{itemize}
...
\begin{parag}{Multi-input linear regression: Training}
    Because the output remains to one dimension, we can use the same least-square loss function as before, and write training as:
	...
	\end{subparag}
	Solving this minimization problem involves computing its derivatives w.r.t. the model parameters ($w$) (by using gradient descent).
\end{parag}
...
\end{framedremark}
The derivative can also be measured for an infinitesimal change in $w$, starting from a point $\overline{w}$, we can write it as:
...
\begin{parag}{Minimizing a function}
    In machine learning, we often seek to minimize a function (e.g., the loss function). To minimize such a function, we seek a point where the derivative vanishes, i.e., we search for $w^{\ast}$ such that 
...
\end{parag}
	...
	\begin{parag}{Properties of gradient}
	    ...
	    \end{itemize}
		You can see with this property why the gradient may be useful when looking for minima of a function. If the gradient gives us the steepest slope of the function at a given point, when going down this slope, this means that we are going \important{down} the fastest way possible.
	    \begin{subparag}{Little remark for the future}
	        This is nice, going down the slope will make our loss function decrease. But what if it converges only to a local minima and not a global one? Doing a gradient descent at a local minima won't work or at least a 'vanilla' gradient descent. A solution to this would be gradient descent with momentum.
	    \end{subparag}
	\end{parag}
		\begin{figure}[h!]
		\centering
	...
	\begin{parag}{Linear regression: Solution}
		...
		\begin{align*} 
			R\left(w\right) = \frac{1}{N} \sum_{i= 1}^{N}\left(x_i^Tw - y_i\right)^2
		\end{align*}
	...
	\begin{parag}{Gradient Computation}
		...
		\begin{subparag}{Proof}
			...
			\end{align*}
			This give us by definition of the gradient:
			\begin{align*} 
	...
	\begin{parag}{Matrix form}
		...
		\begin{subparag}{kind of proof II?}
		    So if I understood correctly what the wiki says, the pseudo-inverse $A^{\dagger}$ and $A^{+}$ is essentially the same. The definition of the pseudo-inverse of a matrix $A$ $A^{+}$ satisfies all of the four criterias, known as the Moore-Penrose condition:
			\begin{align*} 
				A A^{+}A = A\\
				A^{+}A A^{+} = A^{+}\\
				...
				\left(A^{+}A\right)^{\ast} =  A^{+}A
			\end{align*}
			The last two are trivially true as we only deal with real number, note still that this is the definition of Hermitian matrix ($A^{\ast} = A$ where the star mean the transpose conjugate (we take the conjugate of every elements and we then take the transpose of this new matrix, we therefore need that both $A^{\dagger}A$ and $A A^{\dagger}$ are hermitian), but why $X^{\dagger} = \left(X^{T}X\right)^{-1}X^{T}$?
	...
	\begin{parag}{Evaluation metrics for regression}
		...
		\end{itemize}
		We have already spoke about the test data, separate from the training data, doesn't change the parameters of the model... On the other hand, we have never seen any evaluation metrics before, a value that could tell us whether or not our model is performing well out there in the real world.\\
		$ $\\
	...
	\begin{parag}{Mean Absolute Error (MAE)}
		...
		\end{align*}
	    \end{definition}
		Maybe it can be more intuitive to look at the error as a percentage:
	...
	First an important thing to check when testing our model is comparing the error value of on the training data \important{and} for the test data. If those two are close this means that our model effectively can generalize to other data. If for instance our model as a very low error on the training data and a high error for the test data, usually this means that the model is \important{overfitting}.
	\begin{center}
...
Until now we have assumed that the output was a single value, i.e., $y_i \in \mathbb{R}$. But in practice, one may want to output multiple values for a given input, i.e., $y_i \in \mathbb{R}^{C}$ with $C > 1$
\begin{parag}{Model}
    To output multiple values, the linear model cannot just rely on a vector $w$, because the product $w^{T}x_i$ yields a single value. So let us a matrix $W \in \mathbb{R}^{\left(D + 1\right) \times C}$ such that:
	\begin{align*} 
		\hat{y}_i = W^{T}x_t = \begin{pmatrix} w_{\left(1\right)}^{T} \\ w_{\left(2\right)}^{T} \\ \vdots \\ w_{\left(C\right)}^{T}  \end{pmatrix} 
	\end{align*}
	Since the outputs are multi-dimensional, we need to slightly modify the loss function, the multi-dimensional counterpart of the $1$D least-square loss is the squared Euclidean distance. Therefore, we can write multi-output linear regression as:
	\begin{align*} 
		\min_W \sum_{i= 1}^{N}||W^{T}x_i - y_i||^2
	\end{align*}
\end{parag}
\begin{parag}{Multi-output linear regression: Gradient}
    We have our loss function but computing its gradient seems a bit more sophisticated as how we did in the single output linear regression. To compute its gradient, we make use of the following two properties:
	\begin{itemize}
		...
		\item Matrix derivative:
			...
			\end{align*}
	\end{itemize}
	Thus, using the chain rule (and replacing $\left(W^{T}x_i - y_i\right)$) with $\left(X_i^{T}W - y_i^{T}\right)$, we obtain the gradient:
	\begin{align*} 
...
The goal for now will be the same as before, however this time instead of having to output a continuous vector, the output will be a class (which is represented by an integer).
...
\begin{parag}{Example (from Andrew Ng's course)}
	...
	\begin{center}
	\includegraphics[scale=0.2]{screenshots/2026-06-04.png}
	\end{center}
	...
	\end{align*}
	If $\hat{y}$ is small there is a high probability of being not malignant and vis versa.\\
	$ $\\
	...
	This gives us for instance as a decision boundary 
	\begin{center}
	\includegraphics[scale=0.2]{screenshots/2026-06-04_1.png}
	\end{center}
	\begin{subparag}{Remark}
		A general rule when using least-square binary classification is that: the boundary between our two classes is of dimension $D$. When we had data of dimension $1$ the decision boundary was a point (i.e. 0.5 above). If our data are in two dimensions, the boundary will be a line.
	\end{subparag}
