...
\begin{parag}{Motivating example: Curve fitting}
	...
	\end{align*}
	But first let us step back a bit and think on what we just did and \important{why} is it different (we will come back to this later) (I meant like writing in it down on your own).\\
	$ $\\
	So we first apply a linear model to our parameter and then we apply our function to that result:
	\begin{align*} 
...
\begin{parag}{Activation function: Examples}
    There is a lot of activation function out there. The goal of those functions is always to add nonlinearity to our model. There is three main function as such, namely:
	\begin{itemize}
	\item Sigmoid: \begin{align*} f\left(a\right) = \frac{1}{1 + \exp\left(-a\right)} \end{align*}
	\item Hyperbolic tangent:
		...
		\item Rectified Linear Unit (ReLU)
			...
			\end{align*}
	\end{itemize}
	As you can see all of those function has something in common, their slope '\textit{coincide}' with the one of the identity function. The goal of the activation function is to '\textit{replace}' the identity, therefore an important properties that they must hold is that their derivatives is $\geq 0$.
\end{parag}
...
\begin{parag}{$ $}
	Because we use a matrix vector product for each layer, the layers are called \important{fully connected}.\\
	$ $\\
	A thing I found interesting here is how this relates to a classical linear models. If we were to also draw the diagram of a classic linear models meaning 
	\begin{align*} 
		\hat{y} = f\left(W_{\left(1\right)}^{T}x\right)
	\end{align*}
	How would that look like? This architecture output would be the same as the one above except the hidden layer would have only \important{one} hidden unit.
\end{parag}
$ $\\
As we can see here, even with a single hidden layer, one can already model complicated functions. The \important{Universal approximation theorem} states that a sufficiently large feed forward network with a non-linear activation can approximate any continuous function on a compact domain to arbitrary precision. So why would we care about adding more layers? The answer lies on how most real world function are defined. Most of them are not arbitrary continuous function, they are \important{compositions of simpler functions} which in the CNN section we will show that having deep networks naturally models it. But first to understand that let's us try and see what happens. 
...
\end{align*}
Where the dependence of $\hat{y}_i $ on the parameters $W$ is denoted as said before.
...
Something that I want to emphasize here (that I found not clear) is what are we even doing here. So to explain a bit more I will use some stuff from the lecture about autodiff from cs-328. What we were doing above was auto differentiation, there is two main modes \important{forward}, \important{backward}. The one we saw above is backward. How it works is first execute the program and store each result and the computational graph structure (the image below) into a tape (this is our $z_{\left(i\right)}$ above). Then we go back and compute the differentiation using that tape:
\begin{center}
\includegraphics[scale=0.3]{screenshots/2026-04-21_3.png}
\end{center}
The reason on why we need that tape is the chain rule:
\begin{align*} 
	\frac{d f(g(x))}{dx} = \frac{df}{dg}(g(x)) \cdot \frac{dg(x)}{dx}
\end{align*}
In the forward pass we are computing $g\left(x\right)$ and all intermediate values in the computation graph. We stores these values in a tape. Then when doing the backward pass. We already have everything needed in order to apply the chain rule efficiently.
Another great video I recommend in order to better understand how it works 
\begin{center}
	\href{https://www.youtube.com/watch?v=wG_nF1awSSY}{What is Automatic Differentiation? -- Ari Seff}
\end{center}

But let us see an example here:\\
$ $\\
...
We decompose it into elementary operations:
\begin{align*}
a &= x + y \\
b &= a \cdot y
\end{align*}
...
\[
x, y \rightarrow a \rightarrow b
\]

\paragraph{Forward pass}
We compute and store intermediate values:
\begin{align*}
a &= x + y \\
b &= a \cdot y
\end{align*}

These values are stored in a tape.
...
Now apply the chain rule backwards.

Step 1: propagate through multiplication $b = a \cdot y$
\begin{align*}
\frac{db}{da} &= y, \quad \frac{db}{dy}\Big|_{\text{path 1}} = a
\end{align*}

Step 2: propagate through addition $a = x + y$
\begin{align*}
\frac{da}{dx} &= 1, \quad \frac{da}{dy} = 1
\end{align*}

Now combine contributions:

\begin{align*}
\frac{db}{dx} &= \frac{db}{da} \cdot \frac{da}{dx} = y \cdot 1 = y \\
\frac{db}{dy} &= \underbrace{\frac{db}{dy}\Big|_{\text{direct}}}_{a} + \underbrace{\frac{db}{da} \cdot \frac{da}{dy}}_{y \cdot 1}
= a + y
\end{align*}
