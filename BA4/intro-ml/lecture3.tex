...
The principle of gradient-based optimization and more generally iterative optimization is that: we have found a way to have a vetter solution based on a previous one. This means that if we iter on this 'way' a lot of time we will have like better(better(better $\left(\cdots(\text{base})\right)$)) which will give us not necessarly the best solution but a good enough solution.
