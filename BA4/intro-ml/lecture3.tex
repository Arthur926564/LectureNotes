...
\subsection{Loss Function ad Empirical Risk}
Let us have an interlude in our classification goal in order to get more tools to tackle this problem. Note that those tools here (loss functions and empirical risks) are general machine learning concepts used elsewhere.
...
\node (img1) {\includegraphics[width=1.55cm]{screenshots/2026-03-04.png}};
\node[below=6mm of img2] (vdotsL) {\Large$\vdots$};
\node[below=8mm of vdotsL] (img3) {\includegraphics[width=1.95cm]{screenshots/2026-03-04_1.png}};
...
\node[below=11mm of t2] (pvd) {\Large$\vdots$};
...
\begin{parag}{Probabilistic interpretation}
	...
	\end{align*}
	Such a prediction can be interpreted as the probability that $x$ belongs to the positive class (or as a score for the positive class). The probability to belong the negative class is then computed as $1 - \hat{y}$.
\end{parag}
...
\begin{parag}{Logistic regression: Training}
    Given $N$ training samples, to learn the parameters $w$ of the logistic regression model, we need to define a loss function. Instead of using the $L_2$ loss as in the least-square case, this is achieved by making use of the probabilistic interpretation.
	\begin{subparag}{Remark}
	    \begin{itemize}
			...
			\item Nevertheless, this loss remains intuitive: The loss is such that it aims to find the parameters that maximize the probability of the correct class for each training sample
	    \end{itemize}
	\end{subparag}
	...
	\end{align*}
	This loss function is known as the cross-entropy (AICC II mentioned?)
\end{parag}
...
\end{align*}
A very nice thing here for us is that, the \important{cross-entropy is a convex function}. But in this case, its minimization has no closed-form solution. This means that we will need to use an approximation: \important{gradient descent}!!
...
\begin{parag}{Probabilistic Derivation}
	...
	\end{align*}
	What this says to us: given $x$, the label $y$ follows a \important{bernoulli distribution}:
	\begin{align*} 
...
The principle of gradient-based optimization and more generally iterative optimization is to use the fact that we are able to found a better solution based on a previous one. This means that if we iter on this 'way' a lot of time we will have better(better(better $\left(\cdots(\text{base})\right)$)) which will give us not necessarily the best solution but a good enough solution.


\begin{parag}{Minimizing a function}
    With 1D function for which the solution cannot be computed in closed form, one can nonetheless use the derivative of this function to minimize it.
	\begin{itemize}
...
\begin{parag}{Convergence criteria}
    This is nice but we still need potential convergence criteria to be sure that our algorithm works effectively:
	\begin{itemize}
	\item Change in function values less than a given threshold: $\left\|R\left(w_{k-1} - R\left(w_k\right)\right)\right\| < \delta_R$
	\item Change in parameter value less than a given threshold: $\left\|w_{k-1} - w_k\right\| < \delta_w$
	...
	\end{align*}
	The rule mentioned above are more practical criteria. In practice we don't want to compute exact optimality, we so care more about:
	\begin{itemize}
		\item change in objective
		\item change in parameters
	\end{itemize}
	Our current criteria work well \important{because} the function is convex + smooth
	\end{framedremark}
	When minimizing we have to be careful about:
	\begin{subparag}{Influence of $\nu$}
	    Too large steps can lead our algorithm to start jumping around the 'solution' and oscillates till infinity. We are not able to have a magic formula to have a good $\eta$, each $\eta$ is problem-dependent.
	\end{subparag}
...
\begin{parag}{Minimizing a non-convex function}
    As in convex function, we can still use the same iterative algorithm as before. However this time. The output of our algorithm depends now on $w_0$. We can be stuck at local minimum and/or at saddle point.
	\begin{subparag}{Remark}
...
\begin{parag}{Recap: Properties of gradient}
	\begin{itemize}
		\item The gradient at a point $w$ has the direction of greatest increase of the function at $w$
...
\begin{parag}{Gradient descent on multivariate function}
	...
	\begin{itemize}
	    ...
	    \item Thresholding the change in parameters, e.g. $||w_{j-1} - w_k|| < \delta_w$
	\end{itemize}
\end{parag}
\begin{parag}{Minimizing a non-convex function  Example: sin, cos based function of 2 variables}
	Let our function be:
	\begin{align*} 
...
\addplot3[
   ...
   {5 + 0.08*x},
   {sin(deg(5 + 0.22*x)) + cos(deg(5 + 0.20*x))});
...
\begin{itemize}
	\item Different learning rate can yield different results
	\item Different Initializations can yield different results
\end{itemize}

\begin{parag}{Back to Logistic Regression}
    With binary ground-truth label $y_i$, the cross entropy can be written as:
	\begin{align*} 
		R\left(w\right) &= \sum_{i \in \text{positive samples}} \ln\left(\hat{y}_i \right) - \sum_{i \in \text{negative sample}} \ln\left(1 - \hat{y}_i \right)\\
