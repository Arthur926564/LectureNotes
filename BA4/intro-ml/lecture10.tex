...
\begin{parag}{Discussion: Convolution stride}
    The goal of strided convolution is to play a similar role to pooling, i.e., decrease the spatial dimensions, but with learnable parameters. However this can have some issues: as if the stride is greater than 1, we may not cover the input map completely, hence ignoring some of the input values.
	\begin{center}
	\includegraphics[scale=0.6]{screenshots/2026-05-05.png}
	\end{center}
\end{parag}
...
\begin{parag}{Standard architecture}
	...
	\end{subparag}
	\begin{center}
			\includegraphics[width=0.45\textwidth]{figure/2026-04-28.pdf}
	\end{center}
	All of those are image classification networks, so their practical use are similar. The differences are mainly about when you'd pick one over another. ResNet is the one that gets used in practice, for image classification, backbone inside detection/segmentation models, transfer learning starting point.
\end{parag}
...
\begin{parag}{Deep Learning: Tricks of the trade}
	...
	\end{center}
\item Data augmentation e.g. for images, flip, rotate, affine transform,\ldots
	\end{itemize}
