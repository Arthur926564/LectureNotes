...
\begin{parag}{Model complexity}
	...
	\begin{itemize}
	    \item We do not longer assume that the output linearly depends on the input
...
\begin{parag}{k-NN: Training vs test}
	...
	\begin{subparag}{Remark}
	    This behaviour is \important{not} specific to k-NN. It can happen in most machine learning algorithm.
	\end{subparag}
\end{parag}
\begin{framedremark}
...
\begin{parag}{Leave-one-out cross-validation (LOOCV)}
    ...
    \end{center}
	When you have done all points, report the average error. So if we take our 9 samples and we therefore fit 9 models, one for each points, we get the following curve (for a linear model) 
	\begin{center}
	...
	$ $\\
	But is this the best model? Now we can check and be pretty sure about our metrics. For instance, let us use a squared model:
	\begin{center}
...
\begin{parag}{Cross-validation}
	...
	\begin{itemize}
		...
		\item Find the best value for one hyper-parameter (e.g. $k$ in k-NN)
			...
			\end{itemize}
	\end{itemize}
	However, it becomes impractical with too many candidates, for example, one cannot choose which combination of functions to use for features expansion among a very large set.
\end{parag}
...
\begin{parag}{Overfitting with a linear model}
    Because the linear model is simple, overfitting occurs fairly rarely. Nevertheless, if there are fewer training samples than input dimensions ($N \leq D  + 1$), then one can always fit a hyperplan that passes exactly through the training samples.
	\begin{subparag}{Proof}
	    This can easily been check given $N$ points in $D + 1$ dimensions. Our linear model will be given as $w^{T}x + b$. Let us simplify notation by absorbing the bias into the vector $\widetilde{x} = \left(x_1, \ldots, x_D, 1\right)$ then $\widetilde{w} \in \mathbb{R}^{D + 1}$ and our model $f$ is given as:
		\begin{align*} 
		...
		\end{align*}
		After the augmentation with have that each $\widetilde{x}^{\left(i\right)} \in \mathbb{R}^{D + 1}$ Then what we are saying is that we want our model to fit perfectly i.e.:
		\begin{align*} 
		...
		\begin{align*} 
			Xw = y
		\end{align*}
		This implies that we have a linear systems with $N$ equations and $D + 1$ unknowns, this is solvable when $N \leq D + 1$
	\end{subparag}
\end{parag}

\begin{parag}{Penalizing overfitting}
	...
	\end{align*}
		Where $R\left(w\right)$ is the empirical risk and $E_w\left(w\right)$ is the regularizer. $\lambda$ is a hyper-parameter that defines the influence of the regularizer $\left(\lambda > 0\right)$
	\end{parag}
	\begin{parag}{Regularised linear regression}
		One of the most common regularizer is the sum of squares of the weights (there is a probabilistic motivation behind it)
		\begin{align*} 
		...
		\end{subparag}
		\begin{subparag}{Remark}
			For people who have done cs-328, this is the Tikhonov regularization technique that we have seen. In cs-328 the goal of regularization was to \important{improve the condition number} of a problem. The geometric interpretation was that the addition of an identity matrix causes the matrix to scale input vectors uniformly in different directions
			\begin{center}
			\includegraphics[width=0.8\textwidth]{figure/2026-03-24.pdf}
			\end{center}
			And this is the same thing as what we said above, the goal here is to improve the condition number $\implies$ having a matrix that is numerically stable, that can be inverted
		\end{subparag}
	...
	\begin{parag}{Multi-output ridge regression}
		When dealing with multiple output dimensions, the parameters are expressed in matrix form. We can write multi-output ridge regression as:
