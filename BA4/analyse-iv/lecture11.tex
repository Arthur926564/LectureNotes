...
\begin{parag}{Inverse Laplace transform and residue theorem}
	...
	To do so we will define two curves 
		...
		\end{split}\]
		\[\begin{split}
		C_r: \left[\frac{\pi}{2}, \frac{3\pi}{2}\right] &\longmapsto \mathbb{C} \\
	...
	\begin{align*} 
		...
		&= \lim_{z \to -2}  \frac{d}{dz} \left(\frac{e^{zt}}{z + 1}\right) = -e^{-2t}\left(t + 1\right)
	\end{align*}
...
\chapter{Application to partial differential equations (PDES)}
...
$ $\\
Let us summarize before starting the general procedure that we will follow. There is three different '\textit{tools}' that we can use:
\begin{itemize}
	...
	\begin{subparag}{$f$ is even}
		...
		\begin{align*} 
			a_n &= \frac{2}{L}\int_0^{L}f\left(x\right)\cos\left(\frac{2\pi n}{L}x\right)dx\\
				&= \frac{2}{L}\int_{-\frac{L}{2}}^{\frac{L}{2}}f\left(x\right)\cos\left(\frac{ 2\pi n}{L}x\right)dx \mathspace \text{ by } L-\text{periodicity}\\
