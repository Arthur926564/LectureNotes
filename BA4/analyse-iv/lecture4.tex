...
\begin{parag}{Extension of Cauchy's integral formula}
	...
	\end{subparag}
	This gives us another way to compute a maybe 'complicated' integral using something that 'may' be easier.\\
	$ $\\
...
\section{Taylor series in $ \mathbb{R}$}
$ $\\
The goal of Taylor series is to approximate any function using polynomial. The reason for why polynomials is that they are very nice to manipulate (derivative/primitive are very easy to compute) they are continuous, smooth, etc...
\begin{definition}{Taylor Series}
Let $f : \mathbb{R} \to \mathbb{R}$ be a sufficiently regular function. For any $x_0 \in \mathbb{R}$ the \important{Taylor polynomial} of degree $N$ is 
\begin{align*} 
...
\end{theoreme}
What this means is that if I gave you a point and a ball around that point. Then, at each point in this ball, the function at this point is equal to its Taylor series.\\
$ $\\
...
\label{par:Remarks}
    ...
    \begin{enumerate}
        ...
        \item We can connect the Taylor series to Cauchy's integral formula. This would say that $f$ can be written as a power series 
			...
			Where 
			\begin{align*} 
				c_n =  \frac{f^{\left(n\right)\left(z_0\right)}}{n!} =  \frac{1}{2\pi i}\int_{\gamma}\frac{f\left(\zeta\right)}{\left(\zeta - z_0\right)^{n+1}}d\zeta
			\end{align*}
...
The intuition behind Laurent series is that we can change our series in a way so that we can represent function even when the Taylor series is not defined. To do so we will allows our series to have \important{negative} exponent.
...
\begin{parag}{Example}
	...
	\end{align*}
	By Cauchy's integral formula $\int_{\gamma}\frac{f\left(\zeta\right)}{\left(\zeta - z_0\right)^{n + 1}}d\zeta = 0$\\
