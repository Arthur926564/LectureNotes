...
\begin{parag}{Example}
	...
	\begin{align*} 
		\sin\frac{1}{z} = \sum_{n= 0}^{\infty}\frac{\left(-1\right)^{k}}{\left(2n + 1\right)!}\left(\frac{1}{z}\right)^{2n+1}
	\end{align*}
...
\end{align*}
	...
	\begin{align*} 
		c_{-1} &= \frac{1}{\left(m - 1\right)!} \frac{d^{m-1}}{dz^{m-1}} [\left(z - z_0\right)^{m}f(z)]\eval_{z_0}\left(z\right)\\
...
\end{theoreme}
	...
	\begin{parag}{Example}
		...
		\end{align*}
		Let $\gamma$ be a simple closed counterclockwise oriented curve. The question is always the same: what is $\int_{\gamma}fdz$?
		\begin{enumerate}
		...
		\end{enumerate}
		What is important is that when we are asked to compute simple line integral, it is important the divide cases based of the singularities inside of $\gamma$
	\end{parag}
...
\begin{parag}{Example}
	...
	So:
	\begin{align*} 
		\int_{\gamma} fdz =  \pi &=  \int_{L_R}fdz + \int_{C_R}fdz\\
		&= \int_{-R}^{R} \frac{1}{1 + t^2}dt \underbrace{\to}_{R \to \infty} \int_{-\infty}^{\infty}\frac{1}{1 + t^2}dt
	\end{align*}
	On the other hand, 
	\begin{align*} 
		\int_{C_R}fdz =  \int_0^{\pi} \frac{1}{1 + R^2e^{2it}}Rie^{it}dt \approx \frac{\pi}{R} \underbrace{\to}_{R \to \infty} 0
	\end{align*}
	This method works for any integrand that decays faster than $\frac{1}{R}$ as $R \to \infty$. Then by the residue theorem, we found that our integral is given as 
	\begin{align*} 
		\int_{-\infty}^{\infty}\frac{1}{1 + x^2}dx = 2 \pi i \left[\frac{1}{2i}\right] = \pi
	\end{align*}
\end{parag}
