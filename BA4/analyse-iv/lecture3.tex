...
And $\gamma : \left[a, b\right] \to \mathbb{R}^{2}$ is a $C^{1}$ curve with image $\Gamma$ then:
	...
	\end{figure}
	Here the orientation is \important{important}. Second we will need to recall some facts:
	\begin{enumerate}
	    \item The line integral is \important{indepedant} of the parametrisation, but switching orientation creates a negative sign
		\item If $\gamma: \left[c, d\right] \to \left[a, b\right]$ is one-to-one, outo and increasing, then $\gamma$ is a parametrization of $\Gamma$.
	...
	\begin{framedremark}
	The value of this line integral is indepedent of the parametrization of $\gamma$ as long as the orientation is preserved.
	\end{framedremark}

	\begin{parag}{Example}
	    Let $\Gamma$ be the unit half-circle aroung $z= 0$, parametrized counter clockwise:
		\begin{center}
...
\begin{subparag}{Remark}
    Above we used the fundamental theorem of calculus. For more general function, one splits the integrands into real and imaginary part and computes the two real integrals separetely.
\end{subparag}
	\end{parag}
...
\begin{theoreme}
	Let $U \subseteq \mathbb{C}$ open and simply connected set and let $\Gamma$ be a closed and piecewise regular curve inside $U$. Let $f : U \to \mathbb{C}$ be holomorpic. Then:
	\begin{align*} 
...
\begin{parag}{Geometric intuition}
	...
	\begin{subparag}{Why is this true?}
		...
		\end{align*}
		And by the cauchy rieman equations both are equal to $0$. Hence: 
...
\begin{parag}{Example ii}
	...
	$ $\\
	We are looking for a simply connected domain which contains the unit circle \important{and} $f$ is holomorpic. But this is not possible because $f$ is only holomorpic on $\mathbb{C}^{\ast}$ and there is no simply connected domain $U \subseteq \mathbb{C}^{\ast}$ that contains $\Gamma$. Therefore, Cauchy's theorem does not apply. (indeed, as we have calculated above, $\int_\Gamma f\left(z\right)dz \neq 0$)
\end{parag}
...
\begin{parag}{Example iii}
	...
	$ $\\
	For instance let us take the domain $u =  \left\{ \text{Re} > 0\right\}$ which is a simply connected domain which contains $\Gamma$, and where $f$ is holomorpic on $u$. By the cauchy theorem, we have that:
	\begin{align*} 
		\int_\Gamma f\left(z\right)dz = 0
	\end{align*}
\end{parag}
...
\begin{itemize}
	...
	\item Find a simply connected open domain $U \subseteq \mathbb{C}$ such that:
		\begin{itemize}
			...
			\item $f$ is holomorpic on $U$
		\end{itemize}
\end{itemize}

There is an extension of Cauchys' theorem which is nice for application:
\begin{theoreme}
...
$ $\\
If $f$ is holomorpic on our two curves, then:
\begin{align*} 
...
\begin{parag}{How to shows this?}
    Since $f$ is holomorpic on int $\sigma$ and  int$\sigma_2$, by Cauchy's theorem:
	\begin{align*} 
...
\begin{theoreme}
Suppose $D \subseteq \mathbb{C}$ is open and $f : D \to \mathbb{C}$ is holomorpic.\\
$ $\\
If $z_0 \in D$,
\begin{align*} 
...
\begin{parag}{How to show this?}
	...
	\end{align*}
	is holomorpic on $D \setminus \left\{z_0\right\}$. By extended Cauchy's theorem we have:
	\begin{align*} 
