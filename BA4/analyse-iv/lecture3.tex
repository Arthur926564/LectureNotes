...
$ $\\
If $f$ is holomorphic on the annulus $\text{int }\Gamma_0 \setminus \text{int }\Gamma_1$, then:
\begin{align*} 
...
\begin{parag}{How to shows this?}
	First let us do a diagram on what we are trying to do:
	\begin{center}
	\includegraphics[width=0.7\textwidth]{figure/2026-06-01.pdf}
	\end{center}
    Since $f$ is holomorphic on int $\sigma_1$ and  int $\sigma_0$, by Cauchy's theorem:
	\begin{align*} 
	...
	\end{align*}
	Notice the $+ 0$ here represents the part between the two $\sigma$.
	\begin{subparag}{Remark}
	    The minus sign here appeared as we are taking the inside in opposite orientation
	\end{subparag}
	\begin{subparag}{Intuition}
	    Here we are saying that every two curves where one is inside another can be represented by two closed curves where the union of their interior (int $\sigma_1 \cup \text{int }\sigma_0$) is int $\Gamma_1 \setminus $ int $\Gamma_0$. As our function is holomorphic inside our two curves that we have defined, the integral is $0$. And as the sum of our two curves (as integral) corresponds to the integral of the curve of $\partial \text{int }\Gamma_1 \setminus \text{int }\Gamma_0$, we have our equality as $\partial \text{int }\Gamma_1 \setminus \text{int }\Gamma_0 = \gamma_0 \cup \gamma_1$ (and by taking care of orientation).
	\end{subparag}
\end{parag}
...
\begin{parag}{How to show this?}
	...
	\begin{subparag}{Intuition}
		The idea is the same as in the proof above, we are essentially creating a small round between our new '\textit{problem}' and make it arbitrarily small.
	\end{subparag}
\end{parag}
