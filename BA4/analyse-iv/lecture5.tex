...
\begin{parag}{Example II}
	...
	\begin{enumerate}
	    \item Assume $z_0 = 0$, we can observe that $f\left(z\right) = \frac{1}{z} - \frac{1}{z + 1}$. But here we just summoned a known Taylor series $\frac{1}{z - \left(-1\right)}$ Which gives us
			...
			\item  Assume $z_0 = -1$. Using the same trick as in the first point: $f\left(z\right) = -\frac{1}{z - \left(-1\right)} + \frac{1}{z}$. Now if we expand $\frac{1}{z}$ as Taylor series around $-1$:
				\begin{align*} 
...
\begin{parag}{Regular point}
	\begin{definition}{Regular point}
		...
		\end{enumerate}
    \end{definition}
	The intuition behind the second point is that '\textit{if f blows up like $\frac{1}{\left(z - z_0\right)^{m}}$ at $z_0$ then it has a pole of order $m$}' 
