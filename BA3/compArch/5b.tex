...
\draw[dotted, thick,->] (exclusive) edge[bend right] node[right] {BusRd / BusShr} (shared);
...
\end{tikzpicture}
	...
	\begin{parag}{MESI Protocol Shared and Exclusive States}
		In the MESI protocol, the following needs to be considered when a processor reads a data item that was previously not in tis own cache:
		\begin{itemize}
	...
	\begin{parag}{Part I}
		\begin{itemize}
			\item In the following diagrams, you are given the current state (labeled as \important{Old}) of a specific cache line in the four caches C0 to C3
		...
		\begin{itemize}
			...
			\item Specify the memory acess that caused these changes; your answer should be in the form
				\begin{center}
...
\end{tabular}
\end{center}
So here we go from exclusive to shared. The only way of doing that is a BusRd, now we go in our finite state machine diagram and we look for all the possible way of lauching a BusRd. For us, we will chosse P1:Load 0x1000 This way we have, the only way for the C0 to go to the shared state, is to have another processor launch a BusRd, The way of doing that is for instance having the P1 to read the value at 0x1000:
	\begin{center}
\begin{tabular}{|c|cc|cc|cc|cc|c|}
\hline
 & \multicolumn{8}{c|}{Cache} &  \\
...
\end{tabular}
\end{center}
As we can go the same thing from the third processor (both are totaly the same). And as everyone will be in the shared state, someone else would also need to read from memory P2.\\
As the C3 processor as the same state as the C1 processor, we also have the \texttt{P3:Load 0x1000}. 
