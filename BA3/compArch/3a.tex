...
\end{center}
\begin{framedremark}
Here the \textit{performance} of our processor is not really performance but more the frequence of the processor which has stabalize in the current last year.
\end{framedremark}
Processor has improved a \important{lot} in comparisaison of memory. 
\paragraph{Remark}%
\label{par:Remark}
When we say \textit{memory}, we mean \texttt{DRAM}.
...
\begin{parag}{Out goal today}
    What we can do here is: instead of having one memory for everything, we can have \important{two memory}: one that is fast but expensive and one that is slow but cheap.\\
...
\begin{parag}{What memory to use?} 
	When we execute code, there is some thing that we do over and over. On the other side, some code are only executed once. Is there a way that we can \textit{use} this \textit{phenomem} to make memory faster?\\
	...
	\begin{itemize}
	    \item \textbf{Temporal locality}
			...
			\item Spatial locality
				\begin{itemize}
				    \item Data which \important{follow in the memory other data} that are currently used have a higher chance to be used in the future (Code are usually sequential, Data: array)
				\end{itemize}
	\end{itemize}
	However, this is not perfect, this is only a probablistic model. We are only making guess and hoping there were right so that we can win some time.
\end{parag}

\begin{parag}{Our placment policy most be:}
	The fact that we do not give the choice for the programmer is not something that is impartal and always true. 
	For instance in a lot of \important{emebedded system} the programmer have the choice on where to put his data. The fact that this is true for emebedded system  means that: When developing those systems, there is only one person/team that is developing the program. When the program is done, we ship the product and we never heard about it again (hopefully). It is totally legitimate to think that this is the solution.
	However here this is not the case, for us, we don't want to bother programmers about this.
...
The main difference between this and the tree made before is that now as a \important{programmer} I don't know which memory I am using, I am at a level of abstraction above which makes it easier for me to develop software.\\
The idea behind the cache is the fololowing:\\
The processor makes a request for an address, we first check if the address is in the directory if it is \textrightarrow we cut off the main memory and answer ourself (as \texttt{SRAM}).
...
\begin{parag}{Definition of a Cache}
	...
	\begin{itemize}
		...
		\item Web browsers have caches, network routers have routing information and even data caches, DNSs cache frequent names, databases caches queries, even in cs108 we used a cache.
	\end{itemize}
...
\begin{parag}{The question now is how does it works?}
	...
	\end{subparag}
	How do we identify if we have an element?
	To do so, we create a \important{tag} for each elements that, which is stored in a \important{register}. When we are looking for an element, we check all registers and if one of them matched, we have a \important{hit}. Furthermore, we then also know which one it is (because each line is different). Then we can just decode the tag to access the content in the \texttt{SRAM}.\\

	From the beginning of this course we only worked with random access based \important{only from the address} however here this is different:\\
	\begin{itemize}
		\item We have a tag which is \important{not} an index but data, we search based on the content instead of the address $\implies$ \texttt{CAM} (Content addressable memory).
		\item We use a memory that is addressable based on the \important{content of the tag}. 
	\end{itemize}
...
There is three different types of cache organization :
\begin{itemize}
...
The issue here also is that \important{all those comparators} are pretty expensive, for each tag we have a to have a comparator which is pretty costly.
	...
	\end{center}	
	The main character here is the \important{cache controller}.
	It is the one that blocks or not the arrows (the little blue square) that let us cut or not the memory of the data bus. The cache controller here is a finite state machine. In this course we will not build it but as the professor said if we had a lab d we would then implement it in verilog. 
	The important thing here is that this is something that is reacting to some signal and do something based on input etc. We exported in some sense a part of the processor. (we took the part which used to talk to memory).
	...
	\begin{parag}{Fully-Associative Cache}
	    Okay all of this is pretty nice however, there is an issue that has still never been mention, how can we tell to the cache controller when there is a \texttt{hit} or a \texttt{miss}? The easiest way to do so is to put a big \texttt{OR} gate from all the tag.
		 This is \important{slow}. Imagine having an \texttt{OR} gate with a million of input. The time growth of this would be logarithmic which is not good for us. Furthermore, it would be too costly.
		\begin{center}
	...
	\begin{parag}{How can we make it simpler}
		...
		On the right we can do a cheap \texttt{RAM} (compared to \texttt{CAM}) as the direct output of the \texttt{Tag} (have an address of the cache directly as input). 

		For this to works, every element should go into a \important{single place} of the \texttt{RAM}. I take the address from the processor, I figured the index in the cache, we'll get out \important{one} candidate word, we have the Tag here, we'll read and compare the tag and the content, if it is a \texttt{HIT} then we won else we \texttt{MISS}.
		\begin{subparag}{Tag}
	...
	\begin{parag}{How to generate \texttt{Addr} and \texttt{Tag}}
	   ...
	   \begin{center}
	   \includegraphics[scale=0.27]{screenshots/2025-10-29_10.png}
	   \end{center}
	   What we want to do is to go from the address in memory which is a 32 bits memory for instance, into a 10 bits memory which is the cache size. How can we do so? We are \textbf{Hashing}.  We are taking less bits but produce the most different combination possible.\\
	   \begin{subparag}{The simplest hashes}
	       What we want as a hash here is a hash that is the simplest and the \important{fastest}.\\
		   ...
		   \end{center}
		   Is this a reasonable hash? It is, \important{only} if it is uniformly distributed. But when we are writting code or writting data, is the data uniformly distributed?\\
		   No: we are always starting at the bottom of the data, we almost never write something at the address \texttt{0x3000} millions something (at the end of the memory). We need to take in our decisions the fact that the most significant bits are very rarely used. This is the reason why taking the \important{least significant bits} is (as we think) the best way of doing it. 
	   \end{subparag}
	...
	\begin{parag}{Direct-mapped Cache}
		...
		\begin{subparag}{What about it?}
			...
			\important{NO}: we have a \important{lot of constraint}. Here, if we have tow important elements to store in the cache but are in the same hashing index then we are screwed... 
		\end{subparag}
	\end{parag}
	...
	\begin{parag}{Set-Associative Cache}
	   ...
	   \begin{subparag}{Critical path}
	       Here the critical path is in the \important{multiplexer}, furthermore every comparisaiton that is made.
	   \end{subparag}
   \end{parag}
	...
	\end{center}
	\begin{subparag}{professor's question during the lecture}
	...
	\begin{parag}{Validity}
	    All of this is looks pretty good however we have some issue with the implementation, we always said \textit{If there is nothing there then we put something, if there is something then we get and check}, however: how can we know if there is something in the first place?\\
...
\begin{parag}{Write policies}
	...
	\begin{itemize}
		...
		\item \important{Write back} or \important{Copy back}: on a write, data are only updated in the cache (hence, main memory data will become wrong/obsolete)
			\begin{itemize}
				...
				\item (+) we do not have to write evry time into memory
			\end{itemize}
	...
	\begin{framedremark}
	What we are doing here is kind of putting a \textit{flag} that says content has changed and while we don't overwrite in the cache, we don't change the content in memory.
	This is same principle as lazy evaluation in scala (kind of), we don't change anything until we have to.
	\end{framedremark}
\end{parag}
...
This question has less \textit{issues} that comes with it, however this is still a legitimate questions:
\begin{center}
    ...
    \begin{itemize}
		\item \important{Write-allocate}: on a write miss, data are also placed in the cache
			\begin{itemize}
				...
				\item If the processor writes a lot of data that it will never read back, it may \important{unecessarily pollute the cache}
			\end{itemize}
...
\begin{parag}{The '3Cs' of caches miss}
    There is three types of \important{cache miss}
	\begin{enumerate}
		\item \important{Compulsory} \textrightarrow Missed that would also happen in an infinitely large fully associative cache with the same block (also called \important{cold-start} misses or \important{first-reference} misses)
	...
	\end{enumerate}
	Those types of caches miss are useful to understand the \important{source of the limitation of performance}.
\end{parag}
