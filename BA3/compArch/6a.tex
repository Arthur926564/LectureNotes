\subsection{Why Hardware Security}
...
Specification of the threats that a system is protexted against:
\begin{itemize}
	\item \important{Trusted Computing Base}: what is the set of trusted hardware and software components
	\item \important{Security properties}: what the trusted computin base is supposed to guarantee
...
y
\begin{parag}{Dynamix Random-Access Memory}
    \begin{itemize}
...
\begin{itemize}
    \item Allocate large chunks of memory, try many addresses, learn weak cells
	\item Release memory to the OS
	\item  Repeatedly map a file with RW permissions to fill memory with page table entries (PTEs)
...
\begin{parag}{Cache Side-Channel Attacks}
    \begin{itemize}
        ...
        \item Attackers distinguish cache hits and misses via high-resolution timing
    \end{itemize}
\end{parag}
