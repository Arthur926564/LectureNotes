...
\begin{parag}{Needs of multiprogrammed}
	...
	\begin{itemize}
		\item All programs must be written without knowledge of where they will be in memory
			\begin{itemize}
				\item Space allocation may need \important{garbage collection}, \important{moving programs} and \important{data}, etc.
			\end{itemize}
		\item Programs can access only their own data
			\begin{itemize}
			...
			\end{itemize}
	\end{itemize}
	\begin{subparag}{Why is this even more a problem?}
...
\begin{parag}{Segmentation and Paging}
	\begin{subparag}{Segmentation}
		...
		\begin{itemize}
			\item Arbitrary start of a block
...
\begin{parag}{Page Tables Can Be Big}
	...
	There is a big spareness for the table, those table are gigantic but we most of what we'll use is only a thousand.\\
	...
	\end{itemize}
\end{parag}
\begin{parag}{Multilevel (Or hierarchical) page tables}
	...
	\end{center}
	This is kind of weird at first, we are adding a new table, how can we gain time or space by adding something in memory?\\
...
\begin{parag}{But, Two memory accesses every time?}
	So we need to have two memory accesses for each access in memory, this is \important{slow}.
    \begin{center}
...
\begin{parag}{A Specialized 'cache' for the translations}
	...
	\end{center}
	What we want is to do a comparison in parallel.\\
...
\begin{parag}{Memory Protection}
	Typically \important{Page table} entries have several attributes (OS specific):
	\begin{itemize}[label=--]
		\item Valid (to indicate presence in main memory)
		\item Allocated (to indicate existence)
...
\begin{parag}{TLB miss - Revised}
    But here we have to re-change our TLB miss because the address can now be also on the disk. this misses that:\\
	When the OS \important{Reaches the page table} after a TLB miss, now there is a new possibility: the \important{addressed page is on disk}
	\begin{itemize}
...
\begin{parag}{Cache vs. Virtual Memory}
	...
	\begin{subparag}{Virtual Memory}
		\begin{itemize}
			...
			\item 	If main memory is full \textrightarrow evict/swap (LRU, etc.)
...
\begin{itemize}
	...
	\item Additionally, \important{virtual memory} is a form of caching between main memory and secondary storage
