...
\begin{parag}{What should we do now?}
    Now we know how to build a processor so let's care about \important{performance}.
	\begin{center}
...
\begin{parag}{What and Why?}
    So now we can add a big number of transistors. The question we will need to answer is: how can we use them in a way to make our processor faster.
	...
	The better way of doing it is to have a binary tree like of \texttt{OR} gate. This way, instead of having to run in a linear time we can run in a \important{logarithm} one.
	\end{subparag}
...
\begin{parag}{Elapsed Time, CPU Time, ..}
	...
	For Prof. Ienne is an academic person, so he needs to publish a lot of paper \textrightarrow he compiles a lot of latex file. Let us take the command
	\begin{lstlisting}
...
\end{framedremark}
His job is to write latex paper. he needs to be fast at this job (that's why neovim is the best). Every time he is compiling my work, this means that he has to wait the elapsed time \important{staring at his screen compiling latex}. This is some time that he is not using for work.
\vspace{0.5cm}
...
\begin{parag}{Relative Performance}
	...
	\begin{subparag}{Common Performance indices}
		...
		\end{itemize}
		The performance of the operating system is also taking in count. We need a operating system that has a very good cache, etc.
	\end{subparag}
	...
	\begin{subparag}{Personal Remark}
		...
		\end{center}
		A slowdown could hide anywhere in that stack. When you see that System X is slower that System Y, it might be:
		\begin{itemize}
		...
		\item the benchmark's working set doesn't fir in cache on one system
...
\begin{parag}{Relate Performance to Hardware Implementation}
    ...
    \begin{itemize}
		...
		\item \important{Instructions per Cycle (IPC) \textleftarrow 1 / CPI}
			\begin{itemize}
				\item Average instructions executed per cycle
...
\begin{parag}{Many other Considerations Influence the Performance}
	\begin{subparag}{A few random examples}
		Instruction Count depends also from the \important{compiler}. 
		It is more important to have an instrution-set which a compiler can use very effectively (best instructions for the required job) rather than a 'reduced' or 'complex' instruction-set, otherwise the instruction count will be larger than needed.\\
		The CPI depends on the \important{cache performance}, if the overall codes size grows, the cache will be less effective (mor misses)...\\
	...
	\end{subparag}
	\begin{center}
	    \textbf{Look for where most of the time goes}
	\end{center}
...
\begin{itemize}
	\item Performance measurement is all but easy many \important{heterogeneous parameters} come into the picture
