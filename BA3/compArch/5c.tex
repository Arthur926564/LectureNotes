...
\begin{parag}{Ideally: Strict Consistency}
	...
	However in any distributed system is (virtually) \important{impossible to obtain a gloval time}, hence let's forget about it... 
...
\end{parag}
	...
	\begin{parag}{More Practical: Sequence Consistency}
		...
		they do what there \important{load and store} in program order, everything is atomic (no other memory operation is started while the previous has not completed)\\
	...
	First we needed in the first place to reorder instruction at writeback:
	\begin{center}
...
\begin{parag}{Dependences through Memory}
	...
	\end{enumerate}
\end{parag}
\begin{parag}{Load-Store Qeues}
	In practice, the \important{memory part of the ROB} is implemented separately and is called a \important{Load-Store Qeue} (in turn, usually implemented as a Load and Store queues)
   \begin{center}
...
\begin{tikzpicture}[
    code/.style={font=\ttfamily\large, text=blue},
    title/.style={font=\bfseries\Large},
    explain/.style={font=\bfseries\Large, text=red},
    arrow/.style={->, thick, red}
]

% Titles
\node[title] (t1) at (-6,3) {Processor or\\Thread \#1};
\node[title] (t2) at ( 6,3) {Processor or\\Thread \#2};

% Left code (Thread #1)
\node[code, anchor=west] (a0) at (-7,1.5) {A = 0;};
\node[code, anchor=west] (adots) at (-7,0.8) {...};
\node[code, anchor=west] (a1) at (-7,0.1) {A = 1;};
\node[code, anchor=west] (ifb) at (-7,-0.6) {if (B == 0) ...};

% Right code (Thread #2)
\node[code, anchor=west] (b0) at (3,1.5) {B = 0;};
\node[code, anchor=west] (bdots) at (3,0.8) {...};
\node[code, anchor=west] (b1) at (3,0.1) {B = 1;};
\node[code, anchor=west] (ifa) at (3,-0.6) {if (A == 0) ...};

% Center explanation
\node[explain, align=center] (exp) at (0,0.6)
{Now we accept\\that this is\\\textbf{possible!}};

% Curved arrows
\draw[arrow] (ifb.east) to[out=0,in=-120] (a1.south);
\draw[arrow] (ifa.west) to[out=180,in=-60] (b1.south);
...
\begin{parag}{Many Relaxed Consistency Models}
	The goal it to choose consistency models which are \important{efficiently implementable byt do no 'surprise too much' programmer}\\
...
\begin{parag}{Relax Everything: Release Consistency Model}
    Still \important{honour every dependence locally} in a processor, but \important{otherwise co,pletely disregard ordering} accross normal loads and stores\\

	Introduce special synchronization operations that have strict ordering
	\begin{itemize}
		\item Typically some instruction are used to \important{acquire access} (\important{$S_A$}) to a shared variable and enforce the orderings $S_A \to $ W and $S_A \to $ R while other instructions are used to \important{realease access} $\left(S_R\right)$ to a shared variable and enforcethe orderings W \textrightarrow $S_R$ and R \textrightarrow $S_R$
	...
	\end{itemize}
	Put the \important{burden on the programmer/compiler} and be as aggressive as you can in the hardware
...
\begin{parag}{Atomic Instructions}
	...
	\begin{itemize}
		...
		\item \important{Copare-and-swap}: compare a register value to a value in memory addressed by another register, and if they are equal, then swap a thirf register value with the one in memory.
	\end{itemize}
...
\begin{parag}{Consistency is Hard}
    Memory \important{Consistency is hard}, there is subtle interaction between hardware optimizations (e.g., sotre buffers, reordering) and memory models make \important{reasoning about correctness challenging}\\
